    ...
    function populatePropertyDetails(property) {
        ...
        function prevImage() {
            ...
            // Check if amenities is not null or undefined before splitting
if (property.amenities) { // Access the amenities field from the property object
    console.log("Amenities from API:", property.amenities); // Add this line for debugging
    const amenitiesArray = property.amenities.split(',').map(item => item.trim());

    // Map amenity codes to user-friendly labels
    const amenityMappings = {
    LOCATION_TYPE_OCEAN_VIEW: "Location Type: Ocean View",
    LOCATION_TYPE_RESORT: "Location Type: Resort",
    SPORTS_GOLF: "Sports: Golf",
    THEMES_FAMILY: "Themes: Family",
    ATTRACTIONS_MUSEUMS: "Attractions: Museums",
    LEISURE_SHOPPING: "Leisure: Shopping",
    ATTRACTIONS_HEALTH_BEAUTY_SPA: "Attractions: Health & Beauty Spa",
    LEISURE_HORSEBACK_RIDING: "Leisure: Horseback Riding",
    LEISURE_WATER_SPORTS: "Leisure: Water Sports",
    ATTRACTIONS_WATER_PARKS: "Attractions: Water Parks",
    SPORTS_SWIMMING: "Sports: Swimming",
    AMENITIES_HAIR_DRYER: "Amenities: Hair Dryer",
    AMENITIES_INTERNET: "Amenities: Internet",
    AMENITIES_AIR_CONDITIONING: "Amenities: Air Conditioning",
    AMENITIES_IRON_BOARD: "Amenities: Iron & Ironing Board",
    KITCHEN_DINING_DISHWASHER: "Kitchen & Dining: Dishwasher",
    KITCHEN_DINING_MICROWAVE: "Kitchen & Dining: Microwave",
    KITCHEN_DINING_AREA: "Kitchen & Dining: Dining Area",
    ENTERTAINMENT_PING_PONG_TABLE: "Entertainment: Ping Pong Table",
    AMENITIES_PARKING: "Amenities: Parking",
    OUTDOOR_DECK_PATIO_UNCOVERED: "Outdoor: Uncovered Deck/Patio",
    POOL_SPA_HEATED_POOL: "Pool & Spa: Heated Pool"
};

    // Format amenities as a bulleted list with labels
    const amenitiesList = amenitiesArray.map(amenityCode => {
        const label = amenityMappings[amenityCode] || amenityCode.replace(/_/g, ' ');
        return `• ${label}`;
    }).join('\n');

    console.log("Formatted Amenities:", amenitiesList); // Add this line for debugging

    const amenitiesElement = document.querySelector(".amenities");
    if (amenitiesElement) {
        amenitiesElement.innerHTML = `Amenities:\n${amenitiesList}`;
    } else {
        console.error("Amenities element not found in the HTML.");
    }
} else {
    console.error("Amenities data not available in the API response.");
}
        }
    }
