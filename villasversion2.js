...
function populatePropertyDetails(property) {
  ...
  if (amenities) {
    ...
    // Create a list of amenities in a <ul> element
    const amenitiesList = document.createElement('ul');
    amenitiesList.setAttribute('id', 'amenitiesList'); // Set an ID for styling or manipulation

    // Split the amenities string and create list items
    const amenityCodes = amenities.split(',').map(code => code.trim());
    amenityCodes.forEach(amenityCode => {
      const listItem = document.createElement('li');
      listItem.textContent = amenityMappings[amenityCode] || amenityCode;
      amenitiesList.appendChild(listItem);
    });

    // Append the amenities list to the DOM
    document.querySelector(".amenities-container").appendChild(amenitiesList);
  }
