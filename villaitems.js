...
function populatePropertyDetails(property) {
    ...
    // Call the convertAmenitiesToString function from amenities.js to display amenities
    const amenitiesElement = document.querySelector(".amenities");

    // Check if amenities is not null before attempting to split it
    if (amenitiesElement && amenities !== null) {
        const amenitiesText = amenities || "No amenities available"; // Provide a default message if amenities are not present
        amenitiesElement.textContent = `Amenities: ${convertAmenitiesToString(amenitiesText)}`;
    } else if (amenitiesElement) {
        amenitiesElement.textContent = "No amenities available"; // Set a default message if amenities is null
    }

    // Check if images is not null before attempting to split it
    const imagesElement = document.querySelector(".images");
    if (imagesElement && images !== null) {
